...
function DeviationPill({ deviation, threshold }: { deviation: number; threshold: number }) {
  ...
  return (
    <Badge variant={isBreach ? 'danger' : 'success'} size="sm">
      {deviation.toFixed(1)} / {threshold.toFixed(1)}
    </Badge>
  );
}
        ...
        <Card shadow="sm">
          <CardBody className="space-y-4">
            <div className="rounded-lg border border-danger-200 bg-danger-50 px-3 py-2 text-sm text-danger-800">
              Cases are created automatically when verifier deviation exceeds the configured
              threshold.
            </div>
            <input
              value={search}
                      ...
                      return (
                        <tr key={item.id} className="hover:bg-danger-50/30">
                          ...
                          <td className="px-5 py-3 whitespace-nowrap">
                            <DeviationPillMemo
                              deviation={auditCase.deviation}
                              threshold={auditCase.threshold}
                            />
                          </td>
                          <td
                            className="px-5 py-3 text-sm text-danger-700 max-w-[220px] truncate"
                            title={auditCase.reason}
                          >
                            {auditCase.reason}
                          </td>
                          ...
                          <td className="sticky right-0 bg-white px-5 py-3 shadow-[-8px_0_12px_-4px_rgba(0,0,0,0.06)]">
                            <div className="flex flex-col gap-2">
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setNoteByCaseId((prev) => ({
                                    ...prev,
                                    [auditCase.id]: e.target.value,
                                  }))
                                }
                              />

...
function StateBadge({ state }: { state: string }) {
  ...
  return <Badge variant={variants[state] ?? 'danger'} size="sm">{state.replace(/_/g, ' ')}</Badge>;
}
...
function ScorePill({ score }: { score: number | null }) {
  ...
  if (score >= 80) return <Badge variant="success" size="sm">{score.toFixed(1)}</Badge>;
  if (score >= 60) return <Badge variant="warning" size="sm">{score.toFixed(1)}</Badge>;
  return <Badge variant="danger" size="sm">{score.toFixed(1)}</Badge>;
}
...
export default function EscalationQueuePage() {
  ...
  const { data, isPending, isFetching, isError } = useQuery<QueueResponse>({
    ...
    queryFn: () => evaluationsApi.listEscalationQueue(page, 20, debouncedSearch || undefined) as Promise<QueueResponse>,
    ...
    <>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          {total > 0 && <Badge variant="danger" size="md">{total} pending</Badge>}
        </div>

        <Card shadow="xs" className="border-warning-200 bg-warning-50">
          <CardBody>
            <p className="text-sm text-warning-900">
            <strong>What triggers escalation?</strong> Evaluations are automatically escalated when
            the QA deviation from the AI score exceeds the threshold in{' '}
          ...
          <div className="-mx-5 -mb-4 overflow-x-auto">
            ...
            <table className="min-w-full divide-y divide-slate-100">
              <thead className="bg-slate-50">
                <tr>
                  <th className="whitespace-nowrap px-5 py-3 text-left text-2xs font-semibold uppercase tracking-wide text-slate-500">Priority</th>
                  <th className="whitespace-nowrap px-5 py-3 text-left text-2xs font-semibold uppercase tracking-wide text-slate-500">Conversation</th>
                  <th className="whitespace-nowrap px-5 py-3 text-left text-2xs font-semibold uppercase tracking-wide text-slate-500">Channel</th>
                  <th className="whitespace-nowrap px-5 py-3 text-left text-2xs font-semibold uppercase tracking-wide text-slate-500">Agent</th>
                  <th className="whitespace-nowrap px-5 py-3 text-left text-2xs font-semibold uppercase tracking-wide text-slate-500">AI</th>
                  <th className="whitespace-nowrap px-5 py-3 text-left text-2xs font-semibold uppercase tracking-wide text-slate-500">QA</th>
                  <th className="whitespace-nowrap px-5 py-3 text-left text-2xs font-semibold uppercase tracking-wide text-slate-500">Reason</th>
                  <th className="whitespace-nowrap px-5 py-3 text-left text-2xs font-semibold uppercase tracking-wide text-slate-500">State</th>
                  <th className="whitespace-nowrap px-5 py-3 text-left text-2xs font-semibold uppercase tracking-wide text-slate-500">Queued</th>
                  <th className="sticky right-0 bg-slate-50 px-5 py-3 text-left text-2xs font-semibold uppercase tracking-wide text-slate-500 shadow-[-8px_0_12px_-4px_rgba(0,0,0,0.06)]">Action</th>
                </tr>
              </thead>
